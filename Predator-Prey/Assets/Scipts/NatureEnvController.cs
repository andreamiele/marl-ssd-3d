...
public class NatureEnvController : MonoBehaviour
{
    ...
    public float teamCatchReward = 0.5f;
    public float catchRadius     = 6.5f;
    ...
    public void PredatorPreyCollision(Agent caughtPrey, Agent catcherPredator) {
        ...
        // 1) Compter les prédateurs dans le rayon
        int participants = 0;
        foreach (var info in agentsList)
        {
            if (info.agent.CompareTag("Predator") && info.agent.gameObject.activeSelf)
            {
                float d = Vector3.Distance(
                    info.agent.transform.position,
                    caughtPrey.transform.position
                );
                if (d <= catchRadius) participants++;
            }
        }

        // 2) Choisir la bonne récompense
        float predReward = (participants == 1)
            ? soloCatchReward
            : teamCatchReward;
        float preyReward = -predReward;

        // 3) Appliquer à la team ou en solo selon isSingleAgent
        if (!isSingleAgent)
        {
            predatorGroup.AddGroupReward(predReward);
            preyGroup.AddGroupReward(preyReward);
        }
        else
        {
            // chaque prédateur participant reçoit predReward
            foreach (var info in agentsList)
            {
                if (info.agent.CompareTag("Predator") &&
                    Vector3.Distance(
                        info.agent.transform.position,
                        caughtPrey.transform.position
                    ) <= catchRadius)
                {
                    info.agent.AddReward(predReward);
                }
            }
            // la proie reçoit preyReward
            caughtPrey.AddReward(preyReward);
        }

        // (optionnel) logging inference…

        // 4) On “tue” la proie et on termine l’épisode si nécessaire
